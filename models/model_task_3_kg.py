...
class KGRAGModelTask3:
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        batch_search_results_snippets = [
            [
                (search_res["page_name"] + " " + search_res["page_snippet"])
                for search_res in search_results
            ]
            for search_results in batch_search_results
        ]
        final_search_results = []
        for idx, (query_embed, search_results_snippets) in enumerate(
            zip(query_embeddings, batch_search_results_snippets)
        ):
            search_results_embeddings = self.calculate_embeddings(
                search_results_snippets
            )
            page_cosine_scores = (search_results_embeddings * query_embed).sum(1)
            retrieval_idxs: list[int] = list((-page_cosine_scores).argsort()[:40])
            final_retrieval_idxs = []
            total_str_len = 5000000
            for _page_idx in retrieval_idxs:
                curr_len = len(batch_search_results[idx][_page_idx]["page_result"])
                if curr_len > total_str_len:
                    continue
                if curr_len > 240000:
                    continue
                total_str_len -= curr_len
                final_retrieval_idxs.append(_page_idx)
                logger.debug(
                    "Current len / total_len: {}/{}".format(curr_len, total_str_len)
                )
            final_retrieval_idxs = final_retrieval_idxs[:20]
            if len(final_retrieval_idxs) < 10:
                final_retrieval_idxs = retrieval_idxs[:10]
            logger.debug(f"HTML idxs: {final_retrieval_idxs=}")
            final_search_results.append(
                [
                    batch_search_results[idx][_page_idx]
                    for _page_idx in final_retrieval_idxs
                ]
            )
        batch_search_results = final_search_results
        ...
        for _idx, _reasoning_prompt in zip(pred_idxs, reasoning_formatted_prompts):
            response = self.llm.chat.completions.create(
                ...
                messages=_reasoning_prompt,  # Send the formatted prompt as a user message
                n=1,  # Number of output sequences to return for each prompt
                top_p=0.9,  # Cumulative probability of the top tokens to consider
                temperature=0.5,  # Sampling randomness
        ...
        for prompt in formatted_prompts:
            response = self.llm.chat.completions.create(
                ...
                messages=prompt,
                n=1,  # Number of output sequences to return for each prompt.
                top_p=0.9,  # Controls the cumulative probability of the top tokens to consider.
                temperature=0.1,  # Randomness of the sampling.
                max_tokens=50,  # Maximum number of tokens to generate per output sequence.
            )
            responses.append(response)
        ...
        for prompt in formatted_prompts:
            try:
                response = self.llm.chat.completions.create(
                    ...
                    top_p=0.01,  # Controls cumulative probability of top tokens
                    temperature=0.5,  # Sampling randomness
                    max_tokens=512,  # Limit on response token length
                )
