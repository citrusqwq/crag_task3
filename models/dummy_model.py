...
class DummyModel:
    def __init__(self):
        ...
        """
        self.public_test_queries = []
        with open("public_test_queries.txt", "r") as f:
          for line in f.readlines():
            self.public_test_queries.append(line.strip())
        self.validation_queries = []
        with open("validation_queries.txt", "r") as f:
          for line in f.readlines():
            self.validation_queries.append(line.strip())
        self.counter = [0, 0, 0]
    ...
    def generate_answer(
        ...
        answer = "i don't know"
        if query in self.public_test_queries:
          print(f"Query in public test:\n\t{query}")
          self.counter[1] += 1
        if query in self.validation_queries:
          print(f"Query in validation:\n\t{query}")
          self.counter[2] += 1
        self.counter[0] += 1
        print(self.counter)
