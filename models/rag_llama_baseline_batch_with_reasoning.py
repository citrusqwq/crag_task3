...
class ChunkExtractor:
    ...
    def _extract_chunks(self, interaction_id, html_source):
        ...
        for start, end in offsets:
            ...
            while len(sentence) > MAX_CONTEXT_SENTENCE_LENGTH:
                chunks.append(sentence[:MAX_CONTEXT_SENTENCE_LENGTH])
                sentence = sentence[MAX_CONTEXT_SENTENCE_LENGTH:]
...
class BatchReasoningRAGModel:
    ...
    def initialize_models(self):
        ...
        self.model_name = f"large-files/ckpts/Meta-Llama-3-{self.model_size}-Instruct/"
        ...
        sentence_model_path = (
            "large-files/ckpts/sentence-transformers/all-MiniLM-L6-v2"
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        # print(len(batch_interaction_ids))
        # print(len(queries))
        # print(len(batch_search_results))
        ...
        final_answers = [None for _ in range(len(queries))]

        reasoning_answers = []
        for idx, response in enumerate(responses):
            ret_answer = response.outputs[0].text.strip().rstrip()
            if ret_answer == "" or "i don't know" in ret_answer.lower():
                ret_answer = "I don't know."
                final_answers[idx] = ret_answer
            reasoning_answers.append(ret_answer)

        final_formatted_prompts = self.format_final_prompts(
            queries, query_times, batch_retrieval_results, reasoning_answers
        )
        final_responses = self.llm.generate(
            final_formatted_prompts,
            vllm.SamplingParams(
                n=1,  # Number of output sequences to return for each prompt.
                top_p=0.9,  # Float that controls the cumulative probability of the top tokens to consider.
                temperature=0.5,  # Randomness of the sampling
        ...
        for idx, response in enumerate(final_responses):
            ret_final_answer = response.outputs[0].text.strip().rstrip()
            if ret_final_answer == "" or "i don't know" in ret_final_answer.lower():
                ret_final_answer = "I don't know."
            if final_answers[idx] is None:
                final_answers[idx] = ret_final_answer

        if self.return_reasoning:
            return final_answers, reasoning_answers
        else:
            return final_answers

    def format_reasoning_prompts(
        self, queries, query_times, batch_retrieval_results=[]
    ):
        system_prompt = "You are provided with a question and various references. Your task is to generate the answer and the reasoning process of the given question. Your answer should based on the references and your own knowledge. **IMPORTANT: If the references do not contain the necessary information to answer the question and you can't answer it directly based on your knowledge, respond with 'I don't know' only. People may die if you generate the wrong answer.** You **MUST** generate the reasoning process before the answer. Let's think step by step now!"
        formatted_prompts = []
        ...
        for _idx, query in enumerate(queries):
            ...
            user_message += "**Remember your rule**: If the references do not contain the necessary information to answer the question and you can't answer it directly based on your knowledge, respond with 'I don't know'.\n"
            user_message += (
    ...
    def format_final_prompts(
        self, queries, query_times, batch_retrieval_results, batch_reasoning_answers
    ):
        """
        ...
        """
        system_prompt = "You are provided with a question and various references and a reasoning output from another AI agent. Your task is to answer the question succinctly, using the fewest words possible. **IMPORTANT: If the references do not contain the necessary information to answer the question and you can't answer it directly based on your knowledge, respond with 'I don't know'. People may die if you generate the wrong answer.** There is no need to explain the reasoning behind your answers."
        formatted_prompts = []
        ...
        for _idx, query in enumerate(queries):
            ...
            user_message += f"{references}\n------\n\n"
            user_message += "# Reasoning output from another AI agent\n"
            user_message += f"{batch_reasoning_answers[_idx]}\n------\n\n"
