...
class OptimBatchRAGModelWithCalculatorReasoning:
    """
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        for idx, response in enumerate(reasoning_responses):
            ...
            if is_false_premise:
                all_reasoning_outputs[pred_idxs[idx]].append({
                    "Parse success": True,
                    "Parsed result": "invalid question"
                })
                continue
            ...
            try:
                ...
                if "invalid question" in curr_final_answer.lower() or (
                    curr_final_answer.lower() in ["none", "never"]
                ):
                ...
                all_reasoning_outputs[pred_idxs[idx]].append({
                    "Parse success": True,
                    "Parsed result": curr_final_answer.strip()
                })
                continue
            except:
                logger.info(
        ...
        for idx, response in enumerate(responses):
            ...
            if "invalid question" in ret_answer.lower() or (
                ret_answer.lower() in ["none", "never"]
            ):
