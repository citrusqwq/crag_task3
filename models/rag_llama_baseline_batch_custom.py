...
class BatchRAGModelCustom:
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        for response in responses:
            curr_outputs = []
            idk = False
            for sample in response.outputs:
                curr_ret_answer = sample.text.strip().rstrip()
                if curr_ret_answer == "" or "i don't know" in curr_ret_answer.lower():
                    ret_answer = "I don't know."
                    idk = True
                    break
                curr_outputs.append(curr_ret_answer)
            if not idk:
                ret_answer = random.choice(curr_outputs)
            answers.append(ret_answer)
        return answers
    ...
    def format_prompts(self, queries, query_times, batch_retrieval_results=[]):
        ...
        for _idx, query in enumerate(queries):
            ...
            user_message += "**Remember the most important rule**: If the references do not contain the necessary information to answer the question and you can't answer it directly based on your knowledge, respond with 'I don't know'. People may die if you generate the wrong answer! You should think carefully whether you really know the answer.\n"
            user_message += (
